...
SUPPORTED_BEDROCK_EMBEDDING_MODELS = {
    "cohere.embed-english-v3": "Cohere Embed Multilingual",
    "cohere.embed-english-v3": "Cohere Embed English",
    "amazon.titan-embed-text-v1": "Titan Embeddings G1 - Text",
    "amazon.titan-text-express-v1": "Titan Embeddings G1 - Text Express",
    "amazon.titan-text-lite-v1": "Titan Embeddings G1 - Text Lite",
...
class BedrockModel(BaseChatModel):
    ...
    async def chat_stream(self, chat_request: ChatRequest) -> AsyncIterable[bytes]:
        ...
        response = self._invoke_bedrock_stream(chat_request)
        message_id = self.generate_message_id()
    ...
    def _parse_messages(self, chat_request: ChatRequest) -> list[dict]:
        ...
        for message in chat_request.messages:
            if isinstance(message, UserMessage):
                ...
                if content:  # Only add the message if it has content
        ...
        if not messages:
            raise HTTPException(
                status_code=400, detail="No valid messages found in the request"
            )
